    ...
    public class MetricService : MongoCollectionServiceBase<Metric>
    {
        ...
        public async Task<List<Metric>> GetMetricsAsync(int envId, string searchText, int pageIndex, int pageSize)
        {
            if (string.IsNullOrWhiteSpace(searchText))
                ...
                return await _collection.Find(p => p.EnvId == envId && p.Name.ToLower().Contains(searchText.ToLower())).SortByDescending(p => p.UpdatedAt).Skip(pageIndex * pageSize).Limit(pageSize).ToListAsync();
        }
