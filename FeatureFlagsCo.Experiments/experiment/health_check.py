...
from experiment.constants import (AZURE_HEALTH_CHECK_TIMEOUT, DEFAULT_HEALTH_CHECK_TIMEOUT, FMT, get_azure_instance_id)
...
class HealthCheck(RedisStub):
    ...
    def __init__(self,
                 ...
                 engine='azure'):
        super().__init__(redis_host, redis_port, redis_passwd, ssl, mode)
        self._wait_timeout = wait_timeout
        self._server = ServerProxy(rpc_server_url)
        self._trace_logger = trace_health_check_logger if trace_health_check_logger else logging.getLogger(
            'trace_health_check')
        self._debug_logger = debug_health_check_logger if debug_health_check_logger else logging.getLogger(
            'debug_health_check')
        ...
        for i in range(sys.maxsize):
            try:
                ...
                for key, value in dict_receivers.items():
                    dict_value = decode(
                        value)  # {'topic': topic, 'instance': instance, 'datetime': datetime.utcnow().strftime(FMT)}
                    topic, instance_id, pulling_time = dict_value.get('topic', ''), dict_value.get(
                        'instance', ''), dict_value.get('datetime', False)
                        ...
                        if interval >= self._timeout:
                            if self._restart_process(key):
                                ...
                                self._trace_logger.error(
                                    f'No reponse in {topic}-{instance_id} more than {self._timeout}s, {topic}-{instance_id} is restarted by sys!!!',
                                    extra=props)
                            else:
                                raise TimeoutError(
                                    f'No reponse in {topic}-{instance_id} more than {self._timeout}s, {topic}-{instance_id} is halting now, DO RESTART EXPT!!!!!'
                                )
                        else:
                ...
                try:
                    ...
                    self._init__redis_connection(self._redis_host, self._redis_port, self._redis_passwd, self._ssl)
