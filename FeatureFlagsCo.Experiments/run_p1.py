...
from redismq.redis_experiment_imp import \
    P1RedisGetExptRecordingInfoReceiver as redis_p1
...
if __name__ == '__main__':
    ...
    if engine == 'azure':
        ...
        aure_sb_p1(sb_host, sb_sas_policy, sb_sas_key, redis_host, redis_port, redis_passwd, redis_mode) \
            .consume(process_name=process_name,
                     topic=(topic, subscription),
                     prefetch_count=prefetch_count,
                     is_dlq=False)
    elif engine == 'redis':
        redis_p1(redis_host, redis_port, redis_passwd, redis_ssl, redis_mode) \
            .consume(process_name=process_name,
                     topic=topic,
                     fetch_mode='single')
    else:
