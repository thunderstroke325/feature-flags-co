...
from azure_service_bus.azure_service_bus_experiment_imp import \
    P3AzureGetExptFFEventsReceiver as azure_sb_p3
from config.config_handling import get_config_value
from redismq.redis_experiment_imp import \
    P3RedisGetExptFFEventsReceiver as redis_p3
...
if __name__ == '__main__':
    ...
    if engine == 'azure':
        ...
        azure_sb_p3(sb_host, sb_sas_policy, sb_sas_key, redis_host, redis_port, redis_passwd, redis_mode) \
            .consume(process_name=process_name,
                     topic=(topic, subscription),
                     prefetch_count=prefetch_count,
                     is_dlq=False)
    elif engine == 'redis':
        redis_p3(redis_host, redis_port, redis_passwd, redis_ssl, redis_mode) \
            .consume(process_name=process_name,
                     topic=topic,
                     fetch_mode='multi')
    else:
